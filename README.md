...
4. Configure your connector in `app/setup_app/config.py`. I configured MySQL to run on port 3306, but the default port is 3306, which you can easily switch the port to in the code.
5. Install Docker
...
5. (you can skip this, but I prefer it) Change your default port from 3306 by opening `my.cnf` or `mysqld.conf.d`. Add or find the line with `port=3306`, and optionally `bind-address=0.0.0.0`.
